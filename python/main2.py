...
for feature in features:
    if df[feature].isnull().sum() > 0:
        ...
        print(f"{feature} 缺失值已用均值 {mean_value:.2f} 填充")

# 剔除异常值
print("\n处理异常值:")
# 学习时长为负数
negative_duration = (df['章节学习次数'] < 0).sum()
print(f"学习时长为负数的样本数: {negative_duration}")

# 签到率超过100%
over_signin = (df['签到(100%)'] > 100).sum()
print(f"签到率超过100%的样本数: {over_signin}")

# 剔除异常值
mask = (df['章节学习次数'] >= 0) & (df['签到(100%)'] <= 100)
df_cleaned = df[mask]
...
# 使四类样本数量均达到430条
from imblearn.over_sampling import RandomOverSampler
ros = RandomOverSampler(sampling_strategy={0: 430, 1: 430, 2: 430, 3: 430, 4: 430}, random_state=42)
...
grade_dist_cleaned = y_encoded.value_counts().sort_index()
grade_names = [list(grade_mapping.keys())[i] for i in grade_dist_cleaned.index]

plt.bar(grade_names, grade_dist_cleaned.values, color=['#ff6b6b', '#ffa726', '#ffee58', '#66bb6a', '#42a5f5'][:len(grade_names)], alpha=0.8)
plt.title('成绩等级分布', fontsize=16, fontweight='bold')
plt.xlabel('成绩等级')
plt.ylabel('学生人数')
for i, v in enumerate(grade_dist_cleaned.values):
...
plt.subplot(1, 2, 1)
signin_counts = df_cleaned['签到(100%)'].value_counts().sort_index()
plt.pie(signin_counts.values, labels=signin_counts.index, autopct='%1.1f%%', startangle=90)
plt.title('签到完成率分布')

plt.subplot(1, 2, 2)
# 签到与成绩的关系
signin_grade = df_cleaned.groupby('成绩等级')['签到(100%)'].mean().reindex(grade_names)
...
for i, feature in enumerate(features):
    ...
    # 计算每个成绩等级的特征均值
    feature_means = []
    for grade in sorted(y_encoded.unique()):
        grade_mask = y_encoded == grade
        feature_means.append(X.loc[grade_mask, feature].mean())
...
print(f"5. 数据预处理完成: 从{original_count}条原始数据中筛选出{cleaned_count}条有效数据")
print(f"6. 数据平衡完成: 通过随机过采样将样本数从{cleaned_count}条扩展到{len(X_resampled)}条")
print(f"7. 训练测试集划分: 训练集{len(X_train)}条，测试集{len(X_test)}条")
