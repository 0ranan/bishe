  ...
  useEffect(() => {
    const fetchAttendances = async () => {
      try {
        ...
        setError(err instanceof Error ? err.message : '获取签到记录失败');
      } finally {
        setLoading(false);
      }
    };
    ...
    <>
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        ...
        {attendances.length === 0 ? (
          <div className="text-center py-12 text-gray-500">暂无签到记录</div>
        ) : (
          <div className="space-y-4">
            {attendances.map((attendance) => (
              ...
              >
                ...
                <div className="mt-2 text-sm text-gray-600">
                  <div>
                    签到码: <span className="font-medium">{attendance.code}</span>
                  </div>
                  <div>
                    开始时间:{' '}
                    {new Date(attendance.start_time).toLocaleString('zh-CN', {
                      timeZone: 'Asia/Shanghai',
                    })}
                  </div>
                  <div>
                    结束时间:{' '}
                    {new Date(attendance.end_time).toLocaleString('zh-CN', {
                      timeZone: 'Asia/Shanghai',
                    })}
                  </div>
                  <div>
